# This code checks consistency among LRT/QLS/DNN-LRT/DNN-QLS/DNN-LRT-RAW/DNN-QLS-RAW under Ha
# For Fig.5, Fig.S4, Fig.S5, and Tab.2 (wo training/test division)
# source("C:/Users/admin-xw/Documents/CurrentProject/EL-CIMB/Code/Sim_Con_Ha.R")
library(mvtnorm)
library(keras3)
library(tensorflow)
library(abind)
...
gamma.vec <- delta * runif(nsnp, 0.25, 1.25) + (1 - delta) * runif(nsnp, -1.25, -0.25)
...
save.image(file = paste(workpath, "/Result/Fig5.RData", sep = ""))
...
# x11()
setEPS()
postscript(paste(workpath, "/Result/Fig5.eps", sep = ""), width = 7, height = 7)
par(mfrow = c(3, 2))
...
setEPS()
postscript(paste(workpath, "/Result/FigS4.eps", sep = ""), width = 7, height = 7)
par(mfrow = c(3, 2))
...
setEPS()
postscript(paste(workpath, "/Result/FigS5.eps", sep = ""), width = 7, height = 7)
par(mfrow = c(3, 2))
