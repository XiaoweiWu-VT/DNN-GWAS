# This code evaluates LRT/QLS/DNN-LRT/DNN-QLS/DNN-ENS/DNN-LRT-RAW/DNN-QLS-RAW/DNN-LRT-AF/DNN-QLS-AF/DNN-ENS-AF under Ha
# For Tab.3, Tab.4, and Tab.5 (with training/test division)
# source("C:/Users/admin-xw/Documents/CurrentProject/EL-CIMB/Code/Sim_Eva_All_Ha.R")
library(mvtnorm)
library(keras3)
library(tensorflow)
library(abind)
...
k.vec <- c(1, 1.5, 2, 2.5)
power.mat <- matrix(NA, 11, length(k.vec))
rownames(power.mat) = c("LRT", "QLS", "ENS", "DNN-LRT", "DNN-QLS", "DNN-ENS", "DNN-LRT-RAW", "DNN-QLS-RAW", "DNN-LRT-AF", "DNN-QLS-AF", "DNN-ENS-AF")
runt.mat <- matrix(NA, 8, length(k.vec))
rownames(runt.mat) = c("DNN-LRT", "DNN-QLS", "DNN-ENS", "DNN-LRT-RAW", "DNN-QLS-RAW", "DNN-LRT-AF", "DNN-QLS-AF", "DNN-ENS-AF")
accu.mat <- matrix(NA, 8, length(k.vec) * 4)
rownames(accu.mat) = c("DNN-LRT", "DNN-QLS", "DNN-ENS", "DNN-LRT-RAW", "DNN-QLS-RAW", "DNN-LRT-AF", "DNN-QLS-AF", "DNN-ENS-AF")
kk <- 0
for (k in k.vec) {
  kk <- kk + 1
  gamma.vec <- delta * runif(nsnp, 0.5 * k, 0.5 * (k + 1)) + (1 - delta) * runif(nsnp, -0.5 * (k + 1), -0.5 * k)
  ...
  accu.mat[1, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.LRT, rmse.LRT, r2.LRT, correlation.LRT)
  accu.mat[2, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.QLS, rmse.QLS, r2.QLS, correlation.QLS)
  accu.mat[3, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.ENS, rmse.ENS, r2.ENS, correlation.ENS)
  accu.mat[4, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.LRT.RAW, rmse.LRT.RAW, r2.LRT.RAW, correlation.LRT.RAW)
  accu.mat[5, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.QLS.RAW, rmse.QLS.RAW, r2.QLS.RAW, correlation.QLS.RAW)
  accu.mat[6, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.LRT.AF, rmse.LRT.AF, r2.LRT.AF, correlation.LRT.AF)
  accu.mat[7, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.QLS.AF, rmse.QLS.AF, r2.QLS.AF, correlation.QLS.AF)
  accu.mat[8, (4 * (kk - 1) + 1) : (4 * (kk - 1) + 4)] <- c(mae.ENS.AF, rmse.ENS.AF, r2.ENS.AF, correlation.ENS.AF)
  ...
  cat("Power for LRT:", mean(pLRT.vec < 0.05), "\n")
  cat("Power for QLS:", mean(pQLS.vec < 0.05), "\n")
  cat("Power for ENS:", mean(pENS.vec < 0.05), "\n")
  ...
  cat("Power for DNN-LRT:", mean(pDNNLRT.vec < 0.05), "\n")
  cat("Power for DNN-QLS:", mean(pDNNQLS.vec < 0.05), "\n")
  cat("Power for DNN-ENS:", mean(pDNNENS.vec < 0.05), "\n")
  pDNNLRTRAW.vec = pchisq(y.LRT.RAW.pred ^ 2, 1, lower.tail = F)
  pDNNQLSRAW.vec = pchisq(y.QLS.RAW.pred ^ 2, 1, lower.tail = F)
  cat("Power for DNN-LRT-RAW:", mean(pDNNLRTRAW.vec < 0.05), "\n")
  cat("Power for DNN-QLS-RAW:", mean(pDNNQLSRAW.vec < 0.05), "\n")
  ...
  cat("Power for DNN-LRT-AF:", mean(pDNNLRTAF.vec < 0.05), "\n")
  cat("Power for DNN-QLS-AF:", mean(pDNNQLSAF.vec < 0.05), "\n")
  cat("Power for DNN-ENS-AF:", mean(pDNNENSAF.vec < 0.05), "\n")

  power.mat[1, kk] <- mean(pLRT.vec < 0.05)
  power.mat[2, kk] <- mean(pQLS.vec < 0.05)
  power.mat[3, kk] <- mean(pENS.vec < 0.05)
  power.mat[4, kk] <- mean(pDNNLRT.vec < 0.05)
  power.mat[5, kk] <- mean(pDNNQLS.vec < 0.05)
  power.mat[6, kk] <- mean(pDNNENS.vec < 0.05)
  power.mat[7, kk] <- mean(pDNNLRTRAW.vec < 0.05)
  power.mat[8, kk] <- mean(pDNNQLSRAW.vec < 0.05)
  power.mat[9, kk] <- mean(pDNNLRTAF.vec < 0.05)
  power.mat[10, kk] <- mean(pDNNQLSAF.vec < 0.05)
  power.mat[11, kk] <- mean(pDNNENSAF.vec < 0.05)

  runt.mat[1, kk] <- runt.LRT[3]
  runt.mat[2, kk] <- runt.QLS[3]
  runt.mat[3, kk] <- runt.ENS[3]
  runt.mat[4, kk] <- runt.LRT.RAW[3]
  runt.mat[5, kk] <- runt.QLS.RAW[3]
  runt.mat[6, kk] <- runt.LRT.AF[3]
  runt.mat[7, kk] <- runt.QLS.AF[3]
  runt.mat[8, kk] <- runt.ENS.AF[3]

  save.image(file = paste(workpath, "/Result/Tab345_Ha_k", k, ".RData", sep = ""))
}
